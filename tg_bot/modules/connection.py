    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if (var == "no"):
                ...
                update.effective_message.reply_text("Отключены подключения к этому чату для пользователей")
            elif(var == "yes"):
                sql.set_allow_connect_to_chat(chat.id, True)
                update.effective_message.reply_text("Enabled connections to this chat for users")
            else:
                update.effective_message.reply_text("Пожалуйста, введите в группу on/yes/off/no!")
        else:
            update.effective_message.reply_text("Пожалуйста, введите в группу on/yes/off/no!")
    else:
        update.effective_message.reply_text("Пожалуйста, введите в группу on/yes/off/no!")
    ...
    if update.effective_chat.type == 'private':
        if len(args) >= 1:
            try:
                ...
                update.effective_message.reply_text("Указан неверный идентификатор чата!")
            if (bot.get_chat_member(connect_chat, update.effective_message.from_user.id).status in ('administrator', 'creator') or 
                ...
                if connection_status:
                    ...
                    update.effective_message.reply_text("Ошибка подключения!")
            else:
                update.effective_message.reply_text("Подключения к этому чату не разрешены!")
        else:
        ...
        update.effective_message.reply_text("Использование ограничено только личными сообщениями!")
...
def disconnect_chat(bot, update):
    if update.effective_chat.type == 'private':
        ...
        if disconnection_status:
           ...
           update.effective_message.reply_text("Неудачное отключение!")
    else:
        update.effective_message.reply_text("Использование только для PM")
        ...
        if (bot.get_chat_member(conn_id, user_id).status in ('administrator', 'creator') or 
            ...
            if need_admin == True:
                if bot.get_chat_member(conn_id, update.effective_message.from_user.id).status in ('administrator', 'creator') or user_id in SUDO_USERS:
                    ...
                    update.effective_message.reply_text("Вы должны быть админом в связанной группе!")
                    exit(1)
            else:
                return conn_id
        else:
            update.effective_message.reply_text("Группа сменила права подключения или вы больше не администратор. \nЯ отключу вас.")
...
Доступны действия с подключенными группами:
  ...
  - /connect <chatid>: подключиться к удаленному чату
  - /disconnect: отключиться от чата
  - /allowconnect on/yes/off/no: разрешить подключение пользователей к группе
"""

__mod_name__ = "Коннект"
